...
4. Codex Host 通过 Document Capability 对唯一 Drive Hub 执行 search/list → create/replace → get → permission verify。Git-authoritative 文档使用 company readable，协作草稿使用 company editable。
5. 回读后用 `acknowledge` 记录 revision、source fingerprint 和 provider ref。provider ref 只进 Host-local state。
6. 对 Feishu-authoritative 草稿运行 `capture-draft`。成功表现是 Memory Candidate/Review 或安全 Outbox 可定位；Git canonical 不被直接修改。
7. 发现 `conflict` 时停止自动发布，使用 `resolve-conflict` 记录 User/Review decision。失败时双方内容都保留。
8. Task 完成后更新 Status/CHANGELOG/Handoff，再同步一次。
