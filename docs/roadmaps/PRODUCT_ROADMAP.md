...
- 当前状态：[TASK-0028](../../tasks/TASK-0028-EARLYMEETING.md) / EarlyMeeting@12c4086 为 In Progress。保留策划、程序、今日交付三区域及工作日 09:45 每群每天一张；仅正式群2增加按名单预建空行，并向本群所有人开放提交和删除。代码与群2协作权限已部署；预填名单3+6人的实际标识匹配待 im:chat:readonly 生效，当前开关关闭。其他群继续本人行权限，各群独立；不追加复杂测试或验收。
...
- 交付边界：个人行 UI 已获 User 验收；新增今日交付已更新到原卡片，尚未取得真实提交或清空证据。测试群保留，两个正式群均已完成本机接入并启用，从 2026-09-08 工作日北京时间 09:45 开始。实际已加载三个调度群，未补发今天的正式群卡片。程序需在 Windows 本机持续运行，没有系统自启，未来准点及正式群发送未到时实测。共享模式不支持按查看者隐藏他人按钮，个人行后端归属校验保留；间歇网络超时根因未确认。正式 Review 前不进入 Done。
