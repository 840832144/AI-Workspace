...
## 2026-09-22 — TASK-0036简表追加等级膨胀

- 两边各追加整数等级膨胀（1级=1），概览仅26个最大Bet解锁等级，明细保留5000级；原四列及dev r7258不变，无新增SVN提交。

## 2026-09-21 — TASK-0036 Bet/EXP提交dev与四列简表

- User批准缺档插值/尾段比例延伸后提交，保留17锚点并消除9处经验倒挂；普通Bet对齐目标，dev r7258仅两个目标文件，远端回读差异0，LevelCfg未改、VIP暂存。
- CF消耗按User确认1/6，简表改为概览/明细两页、两边各四列，保留小数期望及早期差异。完整数值受控保存，Git只记录生成器/方法/验证；PR #10保持OPEN，不冻结/发布/合并/finalize。

## 2026-09-21 — TASK-0036同Bet体验候选

- 固定User修改后的dev r7252，保留LevelCfg；按同金币、冲突较低Bet生成体验/美元曲线与受控数据Excel，17档经验仅作拟议值。整Spin模拟、精确期望、全档经验倒挂及美元差异分别记录，不宣称全部一致；不改源表或提交SVN。
- 原Task/PR #10/reservation沿用，完整数值不进public Git；按工具更新Registry，交User验收与Review。

## 2026-09-20 — TASK-0036返还闭环修订交Review

- 按Dashboard Round 1补齐消费/成长/活动的分母、返还率、净成本和最终完成成本；卡册理论完成与赛季完成率分开，薯片自然160上限与补包成本分开，无消费分母明确N/A，常驻成本未闭合不冒充完成。
- r7013双版本通过896项定向检查、源格与输出一致性及视觉复核；44条相对外链/0外链展示版。原数值报告Accepted保留，当前修订等待Review；无源修改、SVN提交、冻结、发布、合并或finalize。
