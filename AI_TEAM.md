...
- 不保存 credential、访问 token、私钥、个人身份数据或未批准公开的业务明细；RFC-0005 中 User 明确批准的 CR 内容和三个 Top Tycoon 工作簿历史可入库。
- 仅使用完成当前任务所需的最小权限；不得绕过管理员策略或平台安全控制。
- 外部写入、共享权限、部署和发布默认视为有副作用的操作，必须遵循项目授权。
- 日志与报告必须脱敏；只保留复现结论所需的最少证据。
- CR 日常设计/分析工具切换后只写 `projects/cr/`；公司 SVN 正式配置与其他项目实现保持原真相源，不建立双写副本。
