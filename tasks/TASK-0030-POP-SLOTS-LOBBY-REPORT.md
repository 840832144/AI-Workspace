...
- User 要求补充 DSH 已有 UID/地区规律等机器人发现，以及 CR 需要哪些配置表、如何落地；明确只写策划负责部分。继续 TASK-0030，状态仍为 Review，不重分配编号或重复运行 Registry 门禁。
- 研究提交 `7b88544a560c79c638c84e326555561873279732`：策划版新增五类人物资料规律、原研究对人数/身份/活动的解释与局限。保留25/28记录与总述29份差异，不换算全厅机器人比例，不公开完整人物编号、姓名、金额。
- 私有 CR 提交 `9e4f9e4bc8559f027824116e419f157465308eb5`：增加六类策划配置建议的行含义、字段、原案值、待定项、七类玩家情形预期表现及交付清单。表名是工作名，未核对现行表，不宣称都必须新建；优先复用同用途表。
- 两份云文档各原位追加一次，无新建文档或重传图片；原Pop75块、CR179块保留。新文本39/169项及完整源稿91/269项回读匹配，六图上传内容、双向链接与导航各1处保留，未变更公司内可编辑权限。
- 首次读取时云端已删去旧研究外链句，未恢复；源稿同步该删减。精确内部链接及发布记录仍只保存在私有获准目录。
- 未修改原研究/19条证据/完整CR对照/原迁移建议，未补采、开发CR、生成配置文件、改原需求/配置/SVN或指定程序实现。Subagents: none。

唯一下一步：User / ChatGPT 审阅机器人依据表述与六类策划配置拆分，待讨论项不自动生效。
