...
在 User 实际 Windows 电脑维护“策划 / 程序”本人行同卡填写；续接删除本人行、醒目分区标题，以及多群独立的工作日北京时间 10:00 每群每天一张卡片。当前只接入测试群，正式群等待 User 指定后再启用。

## Scope

2026-09-07 本轮 UI 反馈：保存按钮改为“重新保存”，操作移至第三列，内容框默认一行，缩短行距。User 已要求再向测试群发一张新版供验收，继续该发送授权；不模拟用户操作，不将“没问题就通过”视为已验收。续接本 Task，Registry 再验 14 canonical / 0 collision / valid。
...
不扩展到未配置或未经 User 指定的群；正式群暂不发送。不新建应用、不覆盖原模板、不改为多维表格，不购买/部署公网服务、不设置自启/系统服务、不改全局代理、防火墙或 TLS。旧完整 MVP 不恢复。
