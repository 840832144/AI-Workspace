...
在 User 实际 Windows 电脑维护“策划 / 程序”本人行与“今日交付”共享文本框同卡填写；维护本人删除、醒目分区与新行未提交提醒，以及多群独立的工作日北京时间 09:45 每群每天一张卡片。当前接入测试群及正式群1，正式群1从 2026-09-08 开始，正式群2只预留并关闭。
...
不扩展到未配置或未经 User 指定的群；正式群1按已确认的 2026-09-08 起始日期发送，今天不补发，正式群2关闭。不新建应用、不覆盖原模板、不改为多维表格，不购买/部署公网服务、不设置自启/系统服务、不改全局代理、防火墙或 TLS。旧完整 MVP 不恢复。
...
当前交付 EarlyMeeting@a59abfe / PR #4（核心实现 590ed0d）：v11 已热更新原卡片为策划、程序、今日交付三个同级区域，保留两条个人记录。今日交付为全群共用文本框，所有群成员可提交或清空；个人行仍仅本人操作。工作日北京时间 09:45 每群每天一张，10:15 提醒暂停。现场 CONNECTED / SCHEDULE_CONFIGURED time=09:45 / LAYOUT_UPDATED / MEETING_READY；未模拟交付提交或清空，新增区域不冒称已经用户验收。代码、说明与脱敏证据保留在 EarlyMeeting。Subagents: none。

正式群1已启用：User 本机填 ID 并确认“已入群”，正式群1从 2026-09-08 工作日北京时间 09:45 开始，每天一张；测试群保留，正式群2只预留并关闭。实际重启后 CONNECTED / SCHEDULE_CONFIGURED groups=2 scheduled=2 time=09:45，测试群原消息两行恢复，今天未向正式群补发；未来正式群发送未到时实测。Registry 再验 14 canonical / 0 collision / valid，续接本 Task 为 Review，不另占号。真实群标识不进入 Git。以下为历史迭代记录，以本段与最新 User 决定为准。
