...
在 User 实际 Windows 测试目录接通并诊断长连接与真实表单回调，在指定测试群按“策划 / 程序”两个区域分别“＋添加我的一行”，仅人员与晨会内容两列，自动带入本人身份、保存本人内容并更新同一条消息。工作日 10:00 定时保持关闭。
...
不扩展到未配置的群或生产发布，不实现或启用工作日北京时间 10:00 调度。不新建应用、不覆盖原模板、不改为多维表格，不购买/部署公网服务、不设置自启/系统服务、不改全局代理、防火墙或 TLS。旧完整 MVP 不恢复。
