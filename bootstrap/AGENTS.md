...
项目仓库的一次克隆应包含本全局模板的阅读副本和项目自己的 `AGENTS.md`。项目特定的已批准公开材料、唯一写入位置、历史附件与当前配置边界，以该仓库的现行 RFC / Task / 项目规则为准；不要复制到 Host 全局规则中。凭据、账号及私有 Registry 保护不因项目迁移而降低。
...
- 当前 `WRITE` 实现：`create_document`、`register_document`、`append_document`、`replace_document`、`create_folder`
...
10. 新人流程若检测不到 Document Assistant，应在开始采集前尽早报告 `Implementation unavailable`；不得要求新人申请私有源码权限、Clone 私有仓库或自行配置 secret。
