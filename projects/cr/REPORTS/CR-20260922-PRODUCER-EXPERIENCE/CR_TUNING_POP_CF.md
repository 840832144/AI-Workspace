...
> 当前追加结果见[CR同Bet游戏体验](CR_SAME_BET_EXPERIENCE.md)：User批准Bet/EXP提交dev r7258，缺档补齐且倒挂消除；LevelCfg不改、VIP暂存。简表已按CF折扣1/6比较。下文前期候选及r7237提交均保留历史身份，不覆盖当前授权或版本。
...
> 2026-09-21 User最新决定覆盖本节下方历史映射备选：禁止拉伸，CF已有1–300级与CR同级完整对应；仅301–4999的升级成本做拟合，5000为终点。A全段拉伸/B保留前100再拉伸均作废。User已授权仅等级提交dev，VIP暂存；本节以下原“未选映射不写LevelCfg”是历史阶段条件。

当前方法：保留前300级机器理论净耗目标；对CF最后250–300级的“净耗USD×等级金币倍率”拟合线性趋势并锚定300级，尾部除以CR现有等级金币倍率还原净耗目标。末段原表Bet恒定、归一后线性关系在数值精度内一致；不把300级换币档导致的美元成本变化当成升级难度趋势。根据CR当前Bet/levelExp/95%升级RTP反推整数门槛，保留实际整Spin误差。只写LevelCfg.levelUpExp，其他字段不变。
...
User最终确认已有等级逐级对标、后续拟合，并授权等级提交dev；已仅将LevelCfg的4999个升级门槛格提交r7237，5000级终点不变。VIP暂存，其他配置不提交。见[CR_LEVEL_DEV_RESULT](CR_LEVEL_DEV_RESULT.md)。以下旧候选结果保留历史身份。

## 2026-09-21首版实施结果（历史，A/B已作废）
