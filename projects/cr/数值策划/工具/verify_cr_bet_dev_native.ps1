...
try {
 ...
 if(Test-Path -LiteralPath $receipt){
  ...
  $book.Worksheets.Item(1).Range('A5').Value2="dev r$($result.committed_revision)；仅列最大Bet变化的解锁等级，完整曲线在表后，逐级数据见明细。"
 ...
 foreach($s in $book.Worksheets){
  ...
  $s.Range("C8:E$last").HorizontalAlignment=-4152;$s.Range("I8:K$last").HorizontalAlignment=-4152
  $null=$s.Range("A7:E$last").AutoFilter()
 ...
 $detail=$book.Worksheets.Item(2);$driver=$src.Range('N4');$old=$driver.Value2;$before=$detail.Range('J57').Value2
 $driver.Value2=$old*2;$app.CalculateFull()
 if([Math]::Abs($detail.Range('J57').Value2-$before*2) -gt 0.0000001){throw 'CF discount formula did not respond'}
 $driver.Value2=$old;$app.CalculateFullRebuild()
 if([Math]::Abs($detail.Range('J57').Value2-$before) -gt 0.0000001){throw 'CF discount restore failed'}
 ...
 foreach($view in @(@(1,"A1:K$($data.overview.Count+7)",'overview'),@(2,'A297:K317','boundary300'),@(2,'A4990:K5007','terminal5000'))){
