...
put(calc,'A4',d.rows.map((v,i)=>{const r=i+4,l=i+1,has=l<5000;return [l,has?`=IF(SRC!I${r}=1,SRC!D${r},SRC!D${r}/SRC!C${r})`:'N/A',has?`=ROUNDUP(B${r},0)`:'N/A',has?`=B${r}*SRC!B${r}`:'N/A',has?`=D${r}/(SRC!$AB$5*SRC!H${r})`:'N/A',has?`=E${r}*SRC!$AB$6`:'N/A',has?`=E${r}*SRC!$AB$7`:'N/A',l>=6&&has?`=B${r}-SRC!F${r}`:'N/A',l>=6&&l<=300?`=SRC!E${r}/SRC!M${r}`:'N/A',l>=6&&has?`=SRC!D${r}/SRC!E${r}`:'N/A',`=SRC!B${r}/3`,has?`=SRC!K${r}*SRC!J${r}/SRC!L${r}*SRC!$AB$6`:'N/A'];}));
...
const notes=['CR dev r'+d.revision+'受控候选，供User检查；当前不提交dev。经验按1万金币=1点同比换算，VIP配置不动。','CF到达L7–L50对应出发L6–L49，44段标准化Spin相同；其余明确为缺口或拟合。','净耗保留CR 5% / CF 15%历史模型；未调整RTP。125级后等级膨胀保留dev原节点/数值。'];
const headers=['当前等级','最大解锁Bet','理论Spin','毛下注 USD','CR理论净耗 USD','CF假设净耗 USD','等级膨胀','候选EXP/Spin','候选升级门槛','CF原始/拟合门槛','CF标准化Spin','Spin差','依据'];
function display(levels,start){return levels.map((l,i)=>{const r=l+3,t=i+start;return [l,`=SRC!B${r}`,`=CALC!B${r}`,`=CALC!E${r}`,`=CALC!F${r}`,`=CALC!G${r}`,`=SRC!H${r}`,`=SRC!C${r}`,`=SRC!D${r}`,`=SRC!E${r}`,`=SRC!F${r}`,`=CALC!H${r}`,d.rows[l-1][13]];});}
let s=front('候选说明','CR等级对齐CF：5000级受控候选',notes,['项目','当前方案','验收边界'],[
 ['经验单位','CF原始点数×3÷10000','门槛与每Spin经验同比缩放'],
 ['已知等级','到达L7–L50的44段','源EXP比例102条冲突仍保留；作为User设计规则使用'],
 ['升级拟合','锚定L50，使用L30–L50线性趋势','L51+为拟合，不代表CF实测'],
 ...
 ['下一步','User检查后另行批准dev','本轮不提交SVN、不冻结/发布、不合并/finalize']]);
...
for(const [sheet,end]of [[s,ovEnd],[detail,5007]]){sheet.getRange(`B8:B${end}`).setNumberFormat('#,##0');sheet.getRange(`D8:F${end}`).setNumberFormat('"$"#,##0.00');sheet.getRange(`G8:J${end}`).setNumberFormat('#,##0');sheet.getRange('M:M').format.columnWidth=58;}
...
line(s,'L6–L299升级Spin：候选与最新CF规则/拟合',['CR候选','CF规则/拟合'],Array.from({length:294},(_,i)=>{const r=i+9;return [i+6,`=CALC!B${r}`,`=SRC!F${r}`];}),1,ovEnd+4,'0.0');
