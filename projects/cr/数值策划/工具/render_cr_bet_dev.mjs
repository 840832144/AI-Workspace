...
for(const s of [over,detail]){
 ...
 s.getRange(`A1:I${end}`).format={font:{name:'Microsoft YaHei',size:11,color:'#203449'},columnWidth:17,rowHeight:23};
 for(const c of ['A','F'])s.getRange(`${c}1:${c}${end}`).format.columnWidth=10;
 for(const c of ['B','G'])s.getRange(`${c}1:${c}${end}`).format.columnWidth=22;
 s.getRange(`E1:E${end}`).format.columnWidth=4;
 s.getRange('A2').values=[[s===over?'CR 与 Cash Frenzy：等级体验概览':'CR 与 Cash Frenzy：完整逐级明细']];
 s.getRange('A2:I2').format={font:{bold:true,size:16},rowHeight:31};
 s.getRange('A3').values=[['消耗美金为单级升级的模型净耗USD，CF已按User确认乘1/6；不是累计金额或实付账单。']];
 s.getRange('A4').values=[['CR为整次Spin；CF保留原始小数期望。1–4级沿用User的dev门槛，301级后CF Spin/消耗为N/A。']];
 s.getRange('A5').values=[[s===over?'按Bet解锁、金币价值及升级模式变化点列示；完整曲线在表后，逐级数据见明细。':`CR dev r${d.revision}基线＋本次Bet/EXP候选；LevelCfg不改。5000为终点，无下一次升级成本。`]];
 s.getRange('A3:I5').format={font:{size:10,color:'#526477'},rowHeight:22};
 s.getRange('A6:D6').format={fill:'#E6EFF6',font:{bold:true,color:'#1F4E78'}};
 s.getRange('F6:I6').format={fill:'#FDF0D9',font:{bold:true,color:'#986418'}};
 s.getRange('A6').values=[['CR']];s.getRange('F6').values=[['Cash Frenzy']];
 for(const range of ['A7:D7','F7:I7']){
  s.getRange(range).values=[['等级','最大解锁Bet','Spin','消耗美金']];
  s.getRange(range).format={fill:'#213F59',font:{bold:true,color:'#FFFFFF'},rowHeight:26};
 }
 for(const c of ['A','B','F','G'])s.getRange(`${c}8:${c}${end}`).setNumberFormat('#,##0');
 for(const c of ['C','H'])s.getRange(`${c}8:${c}${end}`).setNumberFormat('#,##0.00');
 for(const c of ['D','I'])s.getRange(`${c}8:${c}${end}`).setNumberFormat('"$"#,##0.0000');
}
...
detail.getRange('F8:I5007').formulas=d.cr.map((v,i)=>{
 const r=i+8,t=i+2;return [`=SRC!A${t}`,`=SRC!D${t}`,`=IF(F${r}>300,"N/A",SRC!G${t})`,`=IF(F${r}>300,"N/A",G${r}*H${r}*SRC!$N$2/(SRC!$N$3*SRC!H${t})*SRC!$N$4)`];
});
for(const [start,end,cols]of [['A','D',['A','B','C','D']],['F','I',['F','G','H','I']]]){
 over.getRange(`${start}8:${end}${d.overview.length+7}`).formulas=d.overview.map(l=>cols.map(c=>`='明细'!${c}${l+7}`));
}
for(const [index,col,other,title,format]of [[0,'B','G','最大解锁Bet（金币）','#,##0'],[1,'C','H','单级升级Spin（CF为原始期望）','0'],[2,'D','I','单级升级净耗USD（CF已乘1/6）','$0.00']]){
